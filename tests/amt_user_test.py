...
def test_user(acct_mgt_url, session):
    ...
    check.is_false(
        amt.ms_check_user(acct_mgt_url, "test01", session),
    ...
    if not amt.oc_resource_exist("users", "User", "test01"):
        check.is_true(
            amt.ms_create_user(acct_mgt_url, "test01", session),
    ...
    if amt.oc_resource_exist("users", "User", "test01"):
        check.is_true(
            amt.ms_delete_user(acct_mgt_url, "test01", session),
            "user test01 deleted",
        )
    check.is_false(
        amt.oc_resource_exist("users", "User", "test01"),
    ...
    if not amt.oc_resource_exist("users", "User", "test01"):
        amt.ms_delete_user(acct_mgt_url, "test01", session)
        # would like to do this in the following manner:
        #
        #   check.is_false(
        #      amt.ms_delete_user(acct_mgt_url, "test01", session),
